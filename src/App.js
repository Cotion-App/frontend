...
function App() {
  ...
  useEffect(() => {
    ...
    const notion_auth = async () => {
      // if the api call fails, clear the search bar link
      const queryParams = new URLSearchParams(window.location.href.split('/')[3])

      if (queryParams.get('code') != null){
        setAuth(true)
        let auth_token = await notionAuth(queryParams.get('code'), 'cotion.herokuapp.com')
    ...
    <div className="App">
      ...
      <div className="content font-serif md:w-4/6 mx-auto p-5 max-w-xl select-none">
        ...
        {!auth ? <div className="notion-auth border-2 border-red-500 rounded-md p-4">
            ...
            <button className="flex items-center" onMouseOver={() => setHovered(true)} onMouseLeave={() => setHovered(false)}  onClick={() => window.open('https://api.notion.com/v1/oauth/authorize?owner=user&client_id=5d758e52-4e9c-4b07-86ad-fd11ee2565aa&redirect_uri=https://google.com&response_type=code',"_self")}>
